"""Add Canvas access token guide document

Revision ID: 007_add_canvas_token_guide
Revises: 006_add_guide_documents
...
revision: str = '007_add_canvas_token_guide'
down_revision: Union[str, None] = '006_add_guide_documents'
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

GUIDE_CONTENT = r"""# Hướng dẫn tạo Access Token trên Canvas LMS
...
def upgrade() -> None:
    escaped = GUIDE_CONTENT.replace("'", "''")
    op.execute(
        "INSERT INTO guide_documents "
        "(panel_key, title, description, icon_name, content, sort_order, is_published) "
        f"VALUES ('canvas_token', 'Hướng dẫn tạo Access Token', "
        f"'Cách tạo và quản lý Access Token trên Canvas LMS', 'Key', "
        f"'{escaped}', 9, true) "
